...
Scene::Scene(GLubyte colorCar[3], GLubyte colorSun[3], GLubyte colorLight[3], GLubyte colorField[3],
    ...
    : camera(),
      ...
      // Scene 3
      funnyTunnel(colorSun, colorField, colorLight, colorBG, &textures),

      // Scene 4
      spiral(colorRoad, colorBG, &textures),
...
void Scene::draw(float time, Shader light)
{
    if (scene == 1)
    {
        ...
        if (player.getCarPos().x > 60.f)
            player.setCarPosX(60.f);
        if (player.getCarPos().x < -60.f)
            player.setCarPosX(-60.f);
    ...
    else if (scene == 4)
    {
        ...
        else
        {
            ...
            if (camera.getCamPos().x < 400)
                camera.setCamPosX(400);
