...
FunnyTunnel::FunnyTunnel(   GLubyte colorTunnel1[3], GLubyte colorTunnel2[3], 
                            GLubyte colorTunnel3[3], GLubyte colorBG[3])
{
    for (int i = 0; i < 3; i++)
    {
...
void FunnyTunnel::draw(float speed, int direction)
{
    ...
    if (auxRot <   0) auxRot = 360 + auxRot;
    if (auxRot > 360) auxRot = auxRot - 360;
    ...
    glLineWidth(2.0f);
    if(starCount == 0) drawMode(GL_LINE, colorTunnel1, 99.8f);
    if(starCount == 1) drawMode(GL_LINE, colorTunnel2, 99.8f);
    if(starCount == 2) drawMode(GL_LINE, colorTunnel3, 99.8f);
}
    ...
    if(!obstacle[0].empty() && speed > 0)
    {
        ...
        while(i < obstacle[0].size())
        {
            if(obstacle[0][i]-500 < 30 + speed && obstacle[0][i]+500 > -30 + speed)
            {
                ...
                if (aux <   0) aux = 360 + aux;
                if (aux > 360) aux = aux - 360;
